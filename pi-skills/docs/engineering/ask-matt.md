...
It **does no work itself**. It doesn't grill, write a PRD, or fix anything — it only orients. It exists for the **user-invoked** skills above all: nothing fires those for you, so *you* have to remember they exist, and `ask-matt` is the memory you offload that to. It also points at the model-invoked skills you'd reach for by name — `/skill:tdd`, `/skill:diagnosing-bugs`, `/skill:prototype`, `/skill:code-review`, and the two vocabulary references, `/skill:domain-modeling` and `/skill:codebase-design`. It answers "which one, and when", then hands you off to the skill that actually does the job.
