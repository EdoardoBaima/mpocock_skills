...
For online material, the subagent should use pi's web tools — `web_search`, `fetch_content`, or `agent_browser` for live docs — to reach the primary source. The search result is not the source; the cited source is the official page, spec, source file, or first-party API that owns the claim.
