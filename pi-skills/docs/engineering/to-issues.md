...
Copy `pi-skills/engineering/to-issues` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/engineering/to-issues ~/.pi/agent/skills/
```

Invoke it as `/skill:to-issues`.

[Pi-adapted source](../../engineering/to-issues/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/engineering/to-issues)
...
You invoke this by typing `/skill:to-issues` — the agent won't reach for it on its own.
...
It sits between [to-prd](https://aihero.dev/skills-to-prd), which hands it a settled spec with user stories to slice against, and [implement](https://aihero.dev/skills-implement), which builds each independently-grabbable issue, driving [tdd](https://aihero.dev/skills-tdd) internally to write the tests test-first, before its [matt-review](https://aihero.dev/skills-code-review) pass. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
