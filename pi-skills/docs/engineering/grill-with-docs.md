...
Copy `pi-skills/engineering/grill-with-docs` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/engineering/grill-with-docs ~/.pi/agent/skills/
```

Invoke it as `/skill:grill-with-docs`.

[Pi-adapted source](../../engineering/grill-with-docs/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/engineering/grill-with-docs)

## What it does

`grill-with-docs` interviews you relentlessly about a plan or design, one frontier round at a time, until you and the agent reach a shared understanding, and it writes the vocabulary and decisions down as you go.
...
- It asks every currently unblocked question in one round, then waits before recomputing the frontier.
