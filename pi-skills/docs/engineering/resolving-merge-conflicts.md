...
Copy `pi-skills/engineering/resolving-merge-conflicts` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/engineering/resolving-merge-conflicts ~/.pi/agent/skills/
```

Invoke it as `/skill:resolving-merge-conflicts`.

[Pi-adapted source](../../engineering/resolving-merge-conflicts/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/engineering/resolving-merge-conflicts)
...
That's why the primary sources matter. You can't preserve an intent you haven't read, so the work starts in the history, such as commits, PRs and tickets, not in the diff.
...
- The project's own typecheck, test and format checks are found and run green before the commit.
