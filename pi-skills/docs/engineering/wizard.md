...
Copy `pi-skills/engineering/wizard` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/engineering/wizard ~/.pi/agent/skills/
```

Invoke it as `/skill:wizard`.

[Pi-adapted source](../../engineering/wizard/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/engineering/wizard)
...
You can type `/skill:wizard`, and the agent can also reach for it on its own. When it hits a step you have to take, such as a key it cannot mint or a dashboard it cannot use, it builds you a wizard instead of writing the instructions into the chat, where they scroll away.
...
A **stage** is one focused task on one screen. The script clears the terminal between stages, so a stage that overflows the screen loses the part that scrolled away. You author stages in dependency order and set `TOTAL_STAGES`, which drives the stage-by-stage progress display.
...
Yes. The skill is model-invoked, so pi can reach for it when setup work hits a manual step. You can also type `/skill:wizard`. The artifact is a plain bash script, so it does not depend on the coding [harness](https://www.aihero.dev/ai-coding-dictionary/harness) that generated it.
...
`engineering/`, as of v1.2.0. It graduated out of the beta bucket and now belongs to the curated pi engineering set. Copy `pi-skills/engineering/wizard` into `~/.pi/agent/skills/`, or install the whole engineering bucket, then run `/reload`. Its behaviour did not change on graduation.
