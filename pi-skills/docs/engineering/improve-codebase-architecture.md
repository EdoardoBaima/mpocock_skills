...
Copy `pi-skills/engineering/improve-codebase-architecture` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/engineering/improve-codebase-architecture ~/.pi/agent/skills/
```

Invoke it as `/skill:improve-codebase-architecture`.

[Pi-adapted source](../../engineering/improve-codebase-architecture/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/engineering/improve-codebase-architecture)
...
Then it stops and asks which one you want to explore. Pick one and it runs the [grilling](https://aihero.dev/skills-grilling) loop over that design. It examines constraints, what sits behind the seam and which tests survive, while updating the domain model as decisions crystallise.
