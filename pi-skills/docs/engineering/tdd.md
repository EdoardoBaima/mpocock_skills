...
Copy `pi-skills/engineering/tdd` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/engineering/tdd ~/.pi/agent/skills/
```

Invoke it as `/skill:tdd`.

[Pi-adapted source](../../engineering/tdd/SKILL.md)
...
Type `/skill:tdd`, or the agent reaches for it automatically when a task fits, building a feature or fixing a bug test-first, or when you ask for red-green work.
...
[implement](https://aihero.dev/skills-implement) is the chain's build step, and it drives `tdd` internally to build each ticket test-first before handing off to [matt-review](https://aihero.dev/skills-code-review), so `tdd` is the engine inside that step rather than a step of its own. You can also reach for it directly, whenever there's a concrete behaviour to build without a full spec. Its other neighbour is [codebase-design](https://aihero.dev/skills-codebase-design), which it leans on to find deep-module seams worth testing at. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
