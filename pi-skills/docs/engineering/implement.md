...
Copy `pi-skills/engineering/implement` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/engineering/implement ~/.pi/agent/skills/
```

Invoke it as `/skill:implement`.

[Pi-adapted source](../../engineering/implement/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/engineering/implement)
...
Around that core it keeps the loop tight. It typechecks often, runs single test files as it goes, and runs the whole suite once at the end. It then closes with a review pass and a commit to the current branch.
...
Reach for it after the work has been specced and sequenced, not before. Its key neighbours are [to-tickets](https://aihero.dev/skills-to-tickets), which produces the independently-grabbable tickets it works through, and [tdd](https://aihero.dev/skills-tdd), which it drives internally to write the tests at each seam before running its own [matt-review](https://aihero.dev/skills-code-review) pass and committing. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
