...
Copy `pi-skills/productivity/wait-what` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/productivity/wait-what ~/.pi/agent/skills/
```

Invoke it as `/skill:wait-what`.

[Pi-adapted source](../../productivity/wait-what/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/productivity/wait-what)
...
The body reuses the leading words already in your global `AGENTS.md` or `CLAUDE.md` and your project's `CONTEXT.md`. ASD-STE100 Simplified Technical English sets the register. The ubiquitous language supplies the nouns. These files reach for the same [tokens](https://www.aihero.dev/ai-coding-dictionary/token), so invoking the skill reminds the agent of an instruction it already has.
