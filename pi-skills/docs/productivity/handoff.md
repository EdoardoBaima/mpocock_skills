...
Copy `pi-skills/productivity/handoff` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/productivity/handoff ~/.pi/agent/skills/
```

Invoke it as `/skill:handoff`.

[Pi-adapted source](../../productivity/handoff/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/productivity/handoff)
...
Reach for this when a conversation has gone long enough that its context is at risk. You may be near a context limit, wrapping for the day or deliberately handing the work to another agent. Use it when you want the thread preserved without dragging the whole transcript along.
