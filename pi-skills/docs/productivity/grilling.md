...
Copy `pi-skills/productivity/grilling` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/productivity/grilling ~/.pi/agent/skills/
```

Invoke it as `/skill:grilling`.

[Pi-adapted source](../../productivity/grilling/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/productivity/grilling)

## What it does

`grilling` is the relentless interview that stress-tests a plan, decision, or idea before action. It walks down the decision tree branch by branch, resolving the dependencies between decisions one at a time until you and the agent share the same understanding.

It asks **one question at a time** and waits for your answer before the next — never a bulk list, which is bewildering. Each question comes with the agent's own recommended answer. It looks up facts the environment can settle, but it brings decisions back to you and waits for your answer. It won't act until you confirm the shared understanding has been reached.
