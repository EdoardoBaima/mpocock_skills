...
Yes, and a large part of the audience does. Add this to your global `AGENTS.md` or `CLAUDE.md`:
...
Into this skill. Round-based questioning shipped briefly as a separate skill, then moved into `grilling` itself, so everything built on the primitive: `grill-me`, `grill-with-docs`, `triage`, `wayfinder`: got it at once. There is no `batch-grill-me` to install, and no separate sequential skill either; the project instruction above is the way back to one-at-a-time.
