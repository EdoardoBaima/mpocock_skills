...
Copy `pi-skills/productivity/teach` into your pi skills directory, then reload pi.

```bash
mkdir -p ~/.pi/agent/skills
cp -R pi-skills/productivity/teach ~/.pi/agent/skills/
```

Invoke it as `/skill:teach`.

[Pi-adapted source](../../productivity/teach/SKILL.md)

[Upstream source](https://github.com/mattpocock/skills/tree/main/skills/productivity/teach)
...
Reach for it when you want to *learn* a topic over time, such as a language, a framework, yoga or theoretical physics, and want the sessions to accumulate rather than evaporate. It is not for a one-off explanation; if you just need something clarified in the moment, ask directly. Reach for `teach` when the learning is a project.
