...
3. **Branch — is this a multi-session build?**
   - **Yes** → **`/skill:to-prd`** (turn the thread into a PRD) → **`/skill:to-issues`** (split the PRD into independently-grabbable issues). Because the issues are independent, **clear context between each one**: start a fresh session per issue, pass it the PRD plus the single issue, and implement with `/skill:tdd` where possible.
   - **No** → implement directly in this context window, using `/skill:tdd` where possible.

### Context hygiene

Keep steps 1–3 in **one unbroken context window** — don't compact or clear until after `/skill:to-issues` — so the grilling, PRD, and issues all build on the same thinking. Each implementation session then starts fresh, working from the issue.
...
- **Bugs and requests piling up** → **`/skill:triage`**. It moves issues through triage roles and produces agent-ready issues for later implementation.
