...
Avoid specific file paths or code snippets — they go stale fast. Exception: if the `/skill:prototype` skill produced code that encodes a decision more precisely than prose can (state machine, reducer, schema, type shape), add a context pointer to where that prototype code lives rather than inlining it.
