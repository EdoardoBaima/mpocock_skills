...
- **Prototype**: Raise the fidelity of the discussion by making a cheap, rough, concrete artifact to react to — an outline, a rough take, a stub, or UI/logic code via the /skill:prototype skill. Creates the prototype as an asset. Use this when "how should it look" or "how should it behave" is the key question.
- **Grilling**: Conversation with the agent. Uses the /skill:grilling and /skill:domain-modeling skills. Asks one question at a time. The default case.
...
1. Run a `/skill:grilling` and `/skill:domain-modeling` session to surface the open decisions. Ask one question at a time.
...
3. Resolve it, invoking skills as needed — including any the `## Notes` block names. If in doubt, use `/skill:grilling` and `/skill:domain-modeling`.
...
> ```
> Invoke /skill:decision-mapping with the map at <path>.
> ```
>
> **Parallel** — paste one line per window, up to all 3:
> ```
> Invoke /skill:decision-mapping with the map at <path>, ticket auth-strategy.
> Invoke /skill:decision-mapping with the map at <path>, ticket cache-layer.
> Invoke /skill:decision-mapping with the map at <path>, ticket rate-limits.
> ```

**No open tickets remain.** The fog is pushed back far enough that the path to the finish line is clear — the map is done. (The initial grilling may also surface no fog at all, in which case there was never a map to build.) Recommend implementing directly, or using `/skill:to-prd` to schedule a multi-session implementation.
